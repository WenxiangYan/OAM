%% Local and Global OAM for vortex-free self-accelerating beams
clear  
clc;
%% Basic parameter  
wave_length=532e-9;
...
%% Theoretical predictions of local and global OAM
num=401; z1=linspace(-f,f,num); t=z1/f+1; 
S0=200e-6;ht=0.5*t;gt=-t.*(t-2); %trajectory with parabolic-linear acceleration in Fig.3(m-p)
...
%% Angular Specturm of Self-accelerating Bessel-like beams
...
%% Direct calculation of global OAM from the angular spectrum
...
if ii == 1
name=' parabolic-linear acceleration with uniform intensity profile.gif';
imwrite(imind,cm,name,'gif', 'LoopCount',inf,'DelayTime',0.1);
else
imwrite(imind,cm,name,'gif','WriteMode','append','DelayTime',0.1);
end
end
%% Computation of OAM using the complex spatial distribution  in real space
...
%% Computation the local OAM of the non-diffracting structure in real space
% Obtain self-accelerating trajectory
width=256;
axis_x=zeros(1,nz_slice);axis_y=zeros(1,nz_slice);
obj_sampling2=2048;
for ii=1:nz_slice
DiffractZd1=DiffractZ3D(obj_sampling2/2-width+1:obj_sampling2/2+width,obj_sampling2/2-width+1:obj_sampling2/2+width,ii)/sqrt(Imax);
I3=abs(DiffractZd1).^2;
[xx yy]=Gravity(I3,10);
axis_x(ii)=xx;
axis_y(ii)=yy;
end
figure;subplot(1,2,1);plot(z_distance_list-f,axis_x);title('X-trajectory');xlabel('z');
subplot(1,2,2);plot(z_distance_list-f,axis_y);title('Y-trajectory');xlabel('z'); 
%  Observation of the non-diffracting area of Self-accelerating Bessel-like beams
for ii=1:nz_slice 
figure(201);
colormap(mycolor);
tt=abs(DiffractZ3D(1024-width+1:1024+width,1024-width+1:1024+width,ii).^2)/Imax; %
xx=axis_x(ii);yy=axis_y(ii);
maskc=CFO_circ_pixel(2*width,2*width,xx-width,yy-width,25);
imagesc(tt.*maskc);hold on
colorbar('FontSize',16);title(['Z=',num2str(z_distance_list(ii)*1000-200),'mm'],'FontSize',18);caxis([0 1]);
axis off
set(gcf,'unit','centimeters','position',[3 3 20 20]); 
set(gca,'Position',[.03 .03 0.83 0.83]); 
...
[imind,cm] = rgb2ind(imind,256);
if ii == 1
name=' the non-diffracting area of parabolic-linear acceleration with uniform intensity profile.gif';
imwrite(imind,cm,name,'gif', 'LoopCount',inf,'DelayTime',0.1);
else
imwrite(imind,cm,name,'gif','WriteMode','append','DelayTime',0.1);
end
end
%% Computation the local OAM of the non-diffracting area of Self-accelerating Bessel-like beams
obj_sampling2=2*width;
d2=linspace(-(obj_sampling2/2-0.5)*pixel_size,(obj_sampling2/2-0.5)*pixel_size,obj_sampling2); 
[X2,Y2]=meshgrid(d2,-d2); 
OAM_local_calculation=zeros(1,nz_slice);
for ii=1:nz_slice
xx=axis_x(ii);yy=axis_y(ii);
maskc=CFO_circ_pixel(2*width,2*width,xx-width,yy-width,25);% aperture
DiffractZd1=maskc.*DiffractZ3D(obj_sampling/2-width+1:obj_sampling/2+width,obj_sampling/2-width+1:obj_sampling/2+width,ii)/sqrt(Imax);
[OAM_density,OAM_per_photon]=OAMz(DiffractZd1,zeros(obj_sampling2,obj_sampling2),zeros(obj_sampling2,obj_sampling2),d2,-d2);
OAM_local_calculation(ii)=OAM_per_photon;
end
figure;plot(z_distance_list-f,OAM_local_calculation,'r');hold on
ylim([0 1.2*max(OAM_local_calculation)]);
title('local OAM calculation');xlabel('z')
plot(z1,OAM_local_theory,'g');legend('local OAM calculation','local OAM theory') 
